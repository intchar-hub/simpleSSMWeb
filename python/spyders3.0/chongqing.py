...
count = 1
url1 = 'https://www.cqu.edu.cn/Channel/yuanshi.html'#院士
...
for al in all:  
    try:
        if al["href"]!=None and al.get("href").find("Content")!=-1:
            h = al.get('href')
        else:
            continue
        if h is None:
...
url2 = 'https://www.cqu.edu.cn/Channel/rencai.html'#杰出人才
...
for line in urls:
    ...
    try:
        ...
        break
    soup = BeautifulSoup(r.text, 'html.parser')
    news = soup.find_all('div', class_="detailpeople-right size12")
    for s in news:
        temp = s.get_text()
